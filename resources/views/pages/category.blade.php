...
@section('content')
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <div class="text-center mb-8">
            <h1 class="text-4xl font-extrabold text-gray-900 mb-2 tracking-tight">{{ ucfirst($category) }} News</h1>
            <p class="text-gray-600 text-lg max-w-xl mx-auto">
                Explore the latest news in {{ ucfirst($category) }}
            </p>
        </div>

        @if (count($articles) === 0)
            <p class="text-center text-gray-500 text-lg">No articles found for this category.</p>
        @else
                    ...
                    <article
                        ...
                        <a href="{{ $article['url'] }}" target="_blank" class="block overflow-hidden">
                            <img src="{{ $imageUrl }}" alt="Thumbnail" loading="lazy"
                                class="h-52 w-full object-cover rounded-t-2xl transform group-hover:scale-105 transition-transform duration-300 ease-in-out"
